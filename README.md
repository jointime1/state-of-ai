...
*Актуально на 14 апреля 2026. Всё проверено по официальной документации
...
**Рендеринг:** `CLAUDE_CODE_NO_FLICKER=1` — alt-screen рендеринг без мерцания (виртуализированный scrollback). `Ctrl+O` — toggle focus view в NO_FLICKER режиме.
...
**Важно:** в v2.1.97–v2.1.101 исправлены критические уязвимости — обход Bash permissions (backslash-escaping, compound commands, env-var префиксы), command injection в LSP binary detection. Обновляйтесь.

**`/doctor`** — диагностика окружения. Нажмите `f` — Claude автоматически исправит найденные проблемы.

### 4.7 3rd-party провайдеры
